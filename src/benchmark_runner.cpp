...
void BenchmarkRunner::runPreprocessingPhase()
{
    ...
    csv << "iteration"
        ...
        << ",scene_file_load_ms"
        << ",palette_overhead_ms"
        << ",data_preparation_loop_ms"
        << ",transform_calculation_total_ms"
        << ",rotation_total_ms"
        << ",data_preparations_total_ms"
        << ",data_preparation_avg_us"
        << ",data_preparation_min_us"
        << ",data_preparation_max_us"
    ...
    for (int iter = 0; iter < iterations; ++iter)
    {
        ...
        app.renderer = new Renderer(app.window, m_cfg.scenePath.c_str(), &dummyDelta, &dummyCaught, &dummyMoved, WIDTH, HEIGHT);
        ...
        if (!isWarmup)
        {
            ...
            csv << (iter - 1)                          // 0-based after warmup
                ...
                << "," << t.sceneFileLoadMs
                << "," << t.paletteOverheadMs
                << "," << t.dataPreparationLoopMs
                << "," << t.transformCalculationTotalMs
                << "," << t.rotationTotalMs
                << "," << t.dataPreparationsTotalMs
                << "," << t.dataPreparationAvgUs
                << "," << t.dataPreparationMinUs
                << "," << t.dataPreparationMaxUs
...
void BenchmarkRunner::runFramePhase()
{
    ...
    app.renderer = new Renderer(app.window, m_cfg.scenePath.c_str(),
